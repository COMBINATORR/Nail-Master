...
When using Sets in component states (e.g. `useState(new Set())`), test assertions must use Set methods like `.has()` or `.size` rather than Array methods like `.includes()` or `.length`. This avoids crashes from TypeErrors.
