...
describe('CalculatorSummary', () => {
  ...
  it('renders selected services and options correctly and enables CTA button', () => {
    ...
    const { container } = render(<CalculatorSummary {...props} />);
    const text = container.textContent;

    // Check services (toLocaleString separators vary by locale)
    expect(screen.getByText('Manicure')).toBeInTheDocument();
    expect(text).toMatch(/5[\s\u00a0\u202f,]*000\s*₸/);

    expect(screen.getByText('Pedicure')).toBeInTheDocument();
    expect(text).toMatch(/7[\s\u00a0\u202f,]*000\s*₸/);

    // Check options (+ and name may be split across nodes)
    expect(screen.getByText((content) => content.includes('French'))).toBeInTheDocument();
    expect(text).toMatch(/\+[\s\u00a0\u202f]*2[\s\u00a0\u202f,]*000\s*₸/);

    // Check total price
    expect(text).toMatch(/14[\s\u00a0\u202f,]*000\s*₸/);
