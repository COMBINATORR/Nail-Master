...
describe('Calculator', () => {
  ...
  it('displays total price and total time correctly', () => {
    ...
    expect(screen.getAllByText((content, el) => el?.tagName === 'SPAN' && content.includes('5') && content.includes('000') && content.includes('₸')).length).toBeGreaterThan(0);
    expect(screen.getAllByText((content) => content.includes('90 min') && (content.includes('≈'))).length).toBeGreaterThan(0);
  });
  ...
  it('renders selected services and options in receipt', () => {
    ...
    // Total price
    expect(screen.getAllByText((content, el) => el?.tagName === 'SPAN' && content.includes('6') && content.includes('000') && content.includes('₸')).length).toBeGreaterThan(0);
    // Service receipt row
    expect(screen.getAllByText('serviceManicureClassicName').length).toBeGreaterThan(0);
    expect(screen.getAllByText((content, el) => el?.tagName === 'SPAN' && content.includes('4') && content.includes('000') && content.includes('₸')).length).toBeGreaterThan(0);
    // Option receipt row
    expect(screen.getByText('+ optManiDesign')).toBeInTheDocument();
    expect(screen.getAllByText((content, el) => el?.tagName === 'SPAN' && content.includes('+') && content.includes('2') && content.includes('000') && content.includes('₸')).length).toBeGreaterThan(0);
  });
