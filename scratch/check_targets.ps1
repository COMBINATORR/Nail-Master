...
if (!(Test-Path $filePath)) {
    Write-Host "ERROR: src/App.jsx not found at relative path!" -ForegroundColor Red
    exit 1
}
...
for ($i = 0; $i -lt $targets.Length; $i++) {
    $t = $targets[$i]
    # Normalize CRLF and LF to avoid line ending mismatches
    $tNorm = $t.Replace("`r`n", "`n").Trim()
    $contentNorm = $content.Replace("`r`n", "`n")
    
    $index = $contentNorm.IndexOf($tNorm)
    if ($index -lt 0) {
        Write-Host "ERROR: Target $($i+1) NOT found!" -ForegroundColor Red
        # Show first 50 chars of target
        Write-Host "Target start: $($tSub = $tNorm.Substring(0, [Math]::Min(100, $tNorm.Length)))" -ForegroundColor Yellow
        exit 1
    } else {
        # Check if it occurs more than once
        $lastIndex = $contentNorm.LastIndexOf($tNorm)
        if ($index -ne $lastIndex) {
            Write-Host "ERROR: Target $($i+1) occurs MULTIPLE times!" -ForegroundColor Red
            exit 1
        }
        Write-Host "SUCCESS: Target $($i+1) found exactly once." -ForegroundColor Green
    }
}

Write-Host "ALL TARGETS VALIDATED SUCCESSFULLY!" -ForegroundColor Green
